...
public class C2SDeathPointTeleportMessage implements Message<C2SDeathPointTeleportMessage> {
    ...
    @Override
    public PacketHandler<C2SDeathPointTeleportMessage> makeHandler() {
    ...
    private static class DeathPointTeleportHandler extends PacketHandler<C2SDeathPointTeleportMessage> {
        ...
        public void handle(C2SDeathPointTeleportMessage packet, Player player, Object gameInstance) {
            this.tryTeleportToDeath(player, packet.clickEvent).ifRight(unit -> {
                ((ServerPlayer) player).server.getCommands().performPrefixedCommand(player.createCommandSourceStack().withMaximumPermission(2), packet.clickEvent.getValue());
            }).ifLeft(problem -> {
