    ...
    public void onPlayerClone(final PlayerEvent.Clone evt) {
        ...
        if(evt.isWasDeath()) {
            ...
            if (ConfigBuildHandler.IGNORE_KEEP_INVENTORY.get() || !player.getServerWorld().getGameRules().getBoolean(GameRules.KEEP_INVENTORY)) {
                ...
                ItemStack itemstack = FilledMapItem.setupNewMap(player.getServerWorld(), (int) player.posX, (int) player.posZ, (byte) 2, true, true);
                FilledMapItem.renderBiomePreviewMap(player.getServerWorld(), itemstack);
